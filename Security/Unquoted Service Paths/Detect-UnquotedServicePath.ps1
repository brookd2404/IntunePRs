<#
.SYNOPSIS
    This script is used to detect if a service has an unquoted path.
.DESCRIPTION
    This script is used to detect if a service has an unquoted path. If it does then the script will return a value of 1 to indicate that a remediation is needed. If the service does not have an unquoted path then the script will return a value of 0 to indicate that a remediation is not needed.
.LINK
...
Write-Log -Message "Proactive Remediation Detection Script Starting" -LogLevel 1 -Component "Detection Script Start"
Write-Log -Message "RegEx to match values: $ValueRegEx" -LogLevel 1 -Component "Detection Script Start"
try {
    ...
    Write-Log -Message "Error: $_" -LogLevel 3 -Component "Detection - Registry Scan"
    Exit 1
}
