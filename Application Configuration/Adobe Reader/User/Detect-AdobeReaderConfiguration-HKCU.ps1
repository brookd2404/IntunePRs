<#
.SYNOPSIS
    This script is used to detect the current configuration of Adobe Reader in the User Context. It is used in the Proactive Remediation process to determine if a remediation is needed.
.DESCRIPTION
    This script is used to detect the current configuration of Adobe Reader in the User Context. It is used in the Proactive Remediation process to determine if a remediation is needed.
.NOTES
...
try {
    ...
    FOREACH ($Key in $HKLMRegMap.Keys) {
        Write-Log -Message "Checking $Key in $($HKLMRegMap[$key]) has a value of $($RegValueMap[$key])" -LogLevel 1 -Component "HKLM Registry Check"
        $RegKey = Get-ItemProperty -Path $HKLMRegMap[$key] -Name $Key -ErrorAction SilentlyContinue
        IF ($null -ne $RegKey) {
            IF ($RegKey.$($key) -eq $RegValueMap[$key]) {
                Write-Log -Message "Registry Key $($RegKey.$($key)) - $Key has a value of $($RegValueMap[$key])" -LogLevel 1 -Component "HKLM Registry Check"
                "Not Needed on $($RegKey[$key]) - $Key as value is $($RegValueMap[$key])!"
            }
            ELSE {
                Write-Log -Message "Registry Key $($RegKey.$($key)) - $Key does not have a value of $($RegValueMap[$key])" -LogLevel 2 -Component "HKLM Registry Check"
                $RemediationRequired += $true
            }
        }
        ELSE {
            Write-Log -Message "Registry Key $($RegKey.$($key)) - $Key does not exist" -LogLevel 1 -Component "HKLM Registry Check"
            $RemediationRequired += $true
        }
    }

    IF ($RemediationRequired -contains $true) {
        Write-Log -Message "Remediation Required on $($RemediationRequired.Count) items" -LogLevel 3 -Component "Remediation Requirement"
        EXIT 1
    }
    ELSE {
        Write-Log -Message "Remediation Not Required" -LogLevel 1 -Component "Remediation Requirement"
        EXIT 0
    }
}
catch {
