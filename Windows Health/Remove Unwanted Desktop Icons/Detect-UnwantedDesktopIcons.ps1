<#
.SYNOPSIS
    This Script is used to run a remediation if required for unwanted desktop icons.
.DESCRIPTION
    This Script is used to run a remediation if required for unwanted desktop icons. It will check the desktop for any shortcuts that match the specified extensions and contain the specified text.
.LINK
    ...
    $files | ForEach-Object {
        ...
        FOREACH ($contain in $contains) {
            IF ($file.Name -match $contain) {
                Write-Log -Message "Found $($file.Name) containing $($contain)" -LogLevel 2 -Component "Found File"
                $remediationArray += $file
            }
        }
    }

    IF ($remediationArray.Count -ge 1) {
        Write-Log -Message "Found $($remediationArray.Count) Desktop Items to remediate" -LogLevel 3 -Component "Get Desktop Items"
        "Found $($remediationArray.Count) Desktop Items to remediate"
        EXIT 1
    }
    ELSE {
        Write-Log -Message "No Desktop Items found to remediate" -LogLevel 1 -Component "Get Desktop Items"
        EXIT 0
    }
